...
import numpy as np
from isegm.model.ops import DistMaps, BatchImageNormalize, BatchImageDeNormalize, ScaleLayer
import pdb
...
class ISModel(nn.Module):
    ...
    def forward(self, image, points):
        #image, prev_mask, image_grad = self.prepare_input(image)
        image, unstdzd_image, prev_masks = self.prepare_input(image)
        
        ## Remove the second channel of prev_masks(prev_edges) if not used
        prev_masks = prev_masks[:,:1,:,:] if not self.with_prev_edges else prev_masks
        disk_map_coord_features = self.get_coord_features(image, prev_masks, points) ##
        coord_features = disk_map_coord_features
        
        ## Denormalize image back to 0-1 instead of have 0 mean 1 sd
        #denormalised_image = self.denormalization(image)
        
        ## Transform coord features --- identity()
        ...
        ## Upsample instances output to image size
        outputs['instances'] = nn.functional.interpolate(outputs['instances'], size=image.size()[2:],
                                                         mode='bilinear', align_corners=True)
        
        outputs['edges'] = nn.functional.interpolate(outputs['edges'], size=image.size()[2:],
                                                         mode='bilinear', align_corners=True)
    ...
    def prepare_input(self, image):
        ...
        if self.with_prev_mask:
            #edges = image[:, 5:, :, :]
            #image_grad = image[:, 4:5, :, :]
            prev_mask = image[:, 3:, :, :]
            image = image[:, :3, :, :]

        stdzd_image = self.normalization(image)
        return stdzd_image, image, prev_mask #image_grad #, edges
