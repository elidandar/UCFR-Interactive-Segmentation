...
def init_experiment(args, model_name):
    ...
    if torch.cuda.is_available():
        if cfg.distributed: # distributed training
            ...
            if cfg.multi_gpu:
                ...
                assert ngpus >= cfg.ngpus, f"Requested {cfg.ngpus} GPUs but only {ngpus} are available."
            cfg.device = torch.device(f'cuda:{cfg.gpu_ids[0]}')
            
    # elif torch.backends.mps.is_available():
    #     # MPS on macOS
    #     cfg.device = torch.device('mps')
    #     cfg.gpu_ids = []  # MPS doesn't support multi-GPU, so leave this empty.
